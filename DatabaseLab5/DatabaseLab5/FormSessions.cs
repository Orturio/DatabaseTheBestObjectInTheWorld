...
namespace DatabaseLab5
{
    public partial class FormSessions : Form
    {
        ...
        private readonly SessionLogic logic;

        public FormSessions(SessionLogic logicS)
        {
            logic = logicS;
            InitializeComponent();
        }
        ...
        {
            var form = Container.Resolve<FormSession>();
        ...
        private void buttonDel_Click(object sender, EventArgs e)
        {
            if (dataGridView.SelectedRows.Count == 1)
            {
                if (MessageBox.Show("Удалить запись", "Вопрос", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        logic.Delete(new SessionBindingModel { Id = id });
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.Message, "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
